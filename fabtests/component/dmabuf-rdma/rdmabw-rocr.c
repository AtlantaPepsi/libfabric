...
static void init_buf(size_t buf_size, char c)
{
	...
	if (buf_location == DEVICE && use_proxy) {
		if (!rocr_alloc_buf(page_size, buf_size, HOST, 0, &proxy_buf.rocr_buf)) {
...
static int init_nic(int nic, char *ibdev_name, int ib_port)
{
	...
	for (i = 0; i < num_gpus; i++)
		CHECK_NULL((bufs[i].mrs[nic] =
				reg_mr(pd, bufs[i].rocr_buf.buf,
				       bufs[i].rocr_buf.size,
				       bufs[i].rocr_buf.base,
				       bufs[i].rocr_buf.location)));
	if (proxy_buf.rocr_buf.buf)
		CHECK_NULL((proxy_buf.mrs[nic] =
				reg_mr(pd, proxy_buf.rocr_buf.buf,
				       proxy_buf.rocr_buf.size,
				       proxy_buf.rocr_buf.base,
				       proxy_buf.rocr_buf.location)));
...
{
	struct ibv_sge list = {
		.addr = (uintptr_t)bufs[gpu].rocr_buf.buf + idx * size,
		.length = size,
			    ...
			    int signaled)
{
	...
	struct ibv_sge list = {
		.addr = (uintptr_t)proxy_buf.rocr_buf.buf + offset,
		.length = proxy_block,
...
int main(int argc, char *argv[])
{
	...
	num_gpus = rocr_init(gpu_dev_nums, enable_multi_gpu);
